...
result5=$(${EXEC} -it postgres_db psql --username postgres -d hello -a -c \
	'CREATE TABLE users (id int, name varchar, address varchar, phone varchar, PRIMARY KEY(id));')
# modify command result output down to simple, usable string
...
result6=$(${EXEC} -it postgres_db psql --username postgres -d hello -a -c \
	"INSERT INTO users(id, name, address, phone) VALUES (1, 'Teresa', '8745 W Geddes Place', '303-594-9683');")
	#"INSERT INTO users(id, name, address, phone) VALUES (1, 'Dave', '8745 W Geddes Place', '303-204-6735');")
# modify command result output down to simple, usable string
result6_mod=$(echo ${result6} | tr '\r\n' ' ' | awk -F\; '{print $2}' | xargs)
if [ "${result6_mod}" == "INSERT 0 1" ]; then
	echo -e "\t** Value store successful **"
else
	echo "\tValue store failure"
	exit 1
fi
